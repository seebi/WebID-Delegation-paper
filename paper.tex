...
This solution has the advantage of reusing the existing WebID protocol in its current state, as well as that it transfers all powers of user $U$ to agent $A$. Therefore, this type of privilege should be limited to agents that are either extensions of $U$ or ones $U$ can or must fully trust anyway.
...
Any server requiring authentication of an agent using WebID must trust the server hosting the WebID profile, since the profile hosting server can change any information in the profile.
Any user profile can then be thought of as being a user according to the origin server i.e. a \textit{user@origin.org:port} - where \textit{origin.org} is the domain of the origin server, and \textit{port} is the port the server is running on.

When the user owns the origin server and there is a one to one relation between the two, as it is the case in a personal FreedomBox, then the origin server is just an extension of the user, and in that case trusting the server is the same as trusting the user.
...
Things get more interesting when an origin server agent $A$ is serving a number of different profiles $P_1$ \ldots $P_n$ each identifying respectively users $U_1$ \ldots $U_n$ and needs to act on behalf of each of these users.
