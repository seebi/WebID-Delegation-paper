...
\begin{abstract}
The WebID protocol enables the global identification of agents in a distributed manner by employing cryptography and Linked Data principles.
It is used currently mostly in scenarios where a client wants to access some protected resources on a server.
In order to decide whether access should be granted, the server itself needs to make requests to other servers and retrieve WebID profiles, which contain relationship definitions between agents.
Currenlty those information needs to be publicly available, which leads to an privacy issue, since agents may want to hide such information from the general audience.
In this paper we propose an extension of the WebID protocol, which allows the delegation of access from a WebID to a third party, e.g. a server application.
We describe further use-cases that led to the development of this extension and we discuss the delegation protocol with two reference implementations.
\end{abstract}
...
Note that fetching a WebID profile for WebID authentication should be done anonymously, for fear of authentication deadlocks.\todo{ST: why that? done -> allowed}
...
\autoref{sec:req} describes some more use-cases and additional requirements which we had in mind for our solution,
...
These applications provide their service to one user only and run on a device which can be accessed by one user only (e.g. a smartphone).
...
A typical application domain for this class of applications is the Social Web more specifically the distributed and semantic Social Web, where multiple services create and manipulate artefacts like notes, comments and images and spin a data network based on a Linked Data infrastructure and shared vocabularies~\cite{tramp-s-2012--a}.

\subsubsection{Vacation Replacement}

In a typical office scenario, employees declare vacation replacements for the time of their holiday in order to achieve an uninterrupted service during that time.
...
Access for \textit{working groups} to a resource is typically specified in a way that each resource guard needs to maintain (owned or cached) a list of group members of this group.
...
An \textit{office role} is a construct where a user can access specific resources because of his working description.
