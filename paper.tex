...
A secretary agent should use its own certificate (see efficiency requirement) and it should be identified and described by its own WebID profile.
...
The simplest solution for any user agent $A$ wishing to act as the user $U$, is for it to create its own certificate $C_u$ with $U$'s WebID URI in the Subject Alternative Name position and add the public key to the user $U$'s profile $P_u$.
By doing this, the agent $A$ can then connect using its personal certificate $C_u$ and $U$'s profile, whenever it wants to work for / as the user $U$.
This solution has the advantage of not making any modifications to the WebID protocol, as well as that it transfers all powers of user $U$ to agent $A$. However, this type of privilege should be limited to agents that are either extensions of $U$ or ones $U$ can or must fully trust anyway.
...
Whenever $A$ connects as $U_1$ to a remote resource $R$, the received data will reflect $R$'s access control policies specific for $U_1$, and thus the data needs to be placed in a separate graph corresponding to $U_1$.
Results for unauthenticated requests are stored in a common public graph.
...
A possible solution involves carefully keeping identified information that was aimed at different users separate, perhaps by creating different graph stores: one for public - unauthenticated - information, and one graph store for each user the server needs to authenticate as.
...
able to specify for each request on behalf of whom it is making it. The Montague FreedomBox could make a request to the Capulet FreedomBox \textit{On-Behalf-Of} Romeo, a few milliseconds later On-Behalf-Of Lord Montague, and so on.
Again as explained in the previous section is is the responsibility of the Montague FreedomBox to keep these information separate, and act with secrecy whenever it is acting on behalf of one or the other of the users. 
...
We need to make this relation explicit by use of a special RDF relation provisionally called \lstinline|cert:secretary|%
...
The remote server can then verify that the identified agent is the secretary of the agent he wishes to act on behalf of (as specified in the \lstinline|On-Behalf-Of| header, by dereferencing that users profile and verifying that the user specifies the \lstinline|cert:secretary| relation there, as it would if Romeo had the FOAF profiles as shown in Listing \ref{listing:webid1}.
...
(4) The guard ask the verification component to verify the secretary WebID which is named in the certificate (5a).
This process is described in~\cite{story-h-2009--a} and is not touched by the delegation extension.
However, since the \lstinline|On-Behalf-Of| header was sent, the verifier component also fetches the WebID of the principal agent which is referred there.
...
An important issue here is, that the authorization has to be done with the principal agent WebID and not with the secretary WebID.
