...
This section organized in multiple subsections which all discuss a possible solution for enabling access delegation but fail in one or more requirement or based on other problems (except the last subsection).
...
Things get more interesting when an origin server agent $A$ is serving a number of different profiles $P_1$ \ldots $P_n$ each identifying the N users $U_1$ \ldots $U_n$ and is acting on behalf of a these users.
