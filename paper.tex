...
WebIDs is usually thought of as a browser authentication method, allowing humans an easy point and click method to authenticate to any remote servers.
...
\lstinputlisting[
    lastline=26,name=webid,style=turtle,float=htb,label=listing:webid1,
...
But when the origin server (OS) is serving a number of different profiles and is acting on behalf of a number of different users, then we have two issues that need to be taken into account: an issue of data perspective and an efficiency issue.

First whenever $OS$ connects as $U_1$ to a service it has to place the data received in a seperate quad store from the one it 
stores data as when connecting to that resources as user $U_2$. This is because the resource may return different representations
depending on who is connecting to it - one representation for close friends perhaps, and one for more distant ones.
 
Secondly, Whenever the Origin Server needs to act on behalf of $U_1$ to a remote server $R$ it will need to open a new TLS connection to that server using the certificate $C_u1$ for that user.
For $N$ users this could require $N$ parallel connections to the same server $S$.
...
\subsection{OntoWiki}
\todo{Phil: Add OntoWiki general description, the usecases for webid delegation as well as a demonstrating screenshot here}
